...
```
┌──(kali㉿kali)-[~/picoctf/forensic3/WebNet0]
└─$ cat picopico.key
-----BEGIN PRIVATE KEY-----
 ...
 ** (wireshark:5029) 10:13:28.983843 [GLib CRITICAL] -- g_hash_table_destroy: assertion 'hash_table != NULL' failed

[1]  + done       wireshark capture.pcap
┌──(kali㉿kali)-[~/picoctf/forensic3/WebNet0]
└─$ ssldump                                
ssldump: listening on eth0
...
ssldump is already the newest version (1.9-1+b1).
ssldump set to manually installed.
...
┌──(kali㉿kali)-[~/picoctf/forensic3/WebNet0]
└─$ ssldump -r capture.pcap -d -k picopico.key | grep pico
